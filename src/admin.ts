...
// 🔴 LOT 163-A — le palier ne se recalcule PAS ici : voir la projection plus bas.
import { paliDe, palierPose, type Compte } from './avoirs.ts';
...
export interface Trouvaille {
  ...
  comptes: {
    ...
    /**
     * 🔴 LOT 163-A — CE QUE LE COMPTE VAUT VRAIMENT, ET CE QUI A ÉTÉ POSÉ.
     * ⭐ LES DEUX, PAS UN SEUL. `palier_effectif` est ce que veveprice
     *   appliquera (le maximum de l'abonnement et de la surcharge) ;
     *   `palier_pose` dit s'il y a une surcharge et jusqu'à quand. Ne montrer
     *   que le premier laisserait un administrateur retirer une surcharge qui
     *   n'existe pas, ou en poser une par-dessus une autre sans le voir.
     * ⛔ Aucun des deux n'est une identité : ce sont des droits, pas des gens.
     */
    palier_effectif: string; palier_pose: string | null; palier_jusqu_a: string | null;
  }[];
}
...
export function chercher(terme: string): Trouvaille {
  ...
  /**
   * ⚠️ LOT 163-A — LES COLONNES SONT NOMMÉES, PAS UN `SELECT *`, et les deux
   *    neuves s'ajoutent DANS LES DEUX BRANCHES. Écrire la liste une fois et
   *    l'oublier dans l'autre rendrait la recherche par portefeuille aveugle
   *    à la surcharge, sans aucune erreur : `undefined` s'affiche « pas de
   *    palier posé », c'est-à-dire une réponse plausible et fausse.
   */
  const COLS = 'id, site, cree_le, verifie, verifie_le, wallet, email, abonne_jusqu_a, '
    + 'supprime_le, palier, palier_jusqu_a';
  const lignes = q<{
    id: string;
    ...
    supprime_le: string | null; palier: string | null; palier_jusqu_a: string | null;
  }>(
    quoi === 'email'
      ? `SELECT ${COLS} FROM comptes WHERE email=? ORDER BY site`
      : `SELECT ${COLS} FROM comptes WHERE wallet=? ORDER BY site`,
    t);
  ...
  return {
    ...
    comptes: lignes.map((c) => ({
      ...
      /**
       * ⭐⭐ ON APPELLE `paliDe()`, ON NE RECALCULE PAS LA RÈGLE ICI. Une
       *   seconde lecture de « qui vaut quoi » divergerait le jour où la
       *   première apprend un cas de plus — c'est la panne du lot 140-1
       *   (trois copies justes de la même loi, jusqu'à ce qu'une seule
       *   apprenne une règle). La page d'exploitation doit afficher ce que
       *   veveprice appliquera, pas ce qu'elle croit qu'il appliquera.
       */
      palier_effectif: paliDe(c as unknown as Compte),
      palier_pose: palierPose(c as unknown as Compte),
      palier_jusqu_a: c.palier_jusqu_a,
    })),
  };
}
