...
COMMIT;`);
  ...
  /**
   * ═══════════════════════════════════════════════════════════════════════
   * 🔴 LOT 163-A — LE PALIER POSÉ ET SA DATE DE FIN.
   * ═══════════════════════════════════════════════════════════════════════
   * ⭐ UN VRAI `ALTER TABLE ADD COLUMN` SUFFIT, ET CE N'EST PAS UNE ENTORSE
   *   au commentaire du lot 89 plus haut. Celui-ci dit que SQLite ne sait pas
   *   RETIRER un `NOT NULL` ni un `UNIQUE` — d'où la recopie. Ajouter une
   *   colonne NULLABLE, il sait faire, et c'est déjà ce que font `email`,
   *   `retour` et `site` juste au-dessus.
   *
   * 🔴 ON RELIT LES COLONNES, ON NE RÉUTILISE PAS `maintenant`. Le bloc `site`
   *   juste au-dessus vient peut-être d'en ajouter une : se fier à un
   *   instantané pris avant lui, c'est la faute que son propre commentaire
   *   décrit (« un instrument branché EN AMONT mesure le passé »).
   *
   * ⚠️ LES DEUX COLONNES SONT AJOUTÉES INDÉPENDAMMENT. Un `if` unique sur la
   *   première laisserait la base à moitié migrée si le service tombe entre
   *   les deux `exec` — et au redémarrage il croirait le travail fait.
   */
  const colsPalier = (db.prepare('PRAGMA table_info(comptes)').all() as Array<{ name: string }>).map((c) => c.name);
  if (!colsPalier.includes('palier')) {
    db.exec('ALTER TABLE comptes ADD COLUMN palier TEXT');
    journal.push('colonne palier ajoutee');
  }
  if (!colsPalier.includes('palier_jusqu_a')) {
    db.exec('ALTER TABLE comptes ADD COLUMN palier_jusqu_a TEXT');
    journal.push('colonne palier_jusqu_a ajoutee');
  }
