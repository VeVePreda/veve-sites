  ...
  if (t && t.quoi === 'inconnu') {
    ...
    r = t.comptes.map((c) => `<div class="carte">
      ...
      c.verifie_le ? ` <span class="doux">(${echapper(c.verifie_le.slice(0, 10))})</span>` : ''}<br>
        ...
        ${/* 🔴 LOT 163-A — LE PALIER EFFECTIF EST TOUJOURS DIT, LA SURCHARGE
              SEULEMENT QUAND ELLE EXISTE. ⭐ C'est la première chose qu'on
              vient vérifier ici, et jusqu'à ce lot elle n'était écrite NULLE
              PART : il fallait ouvrir une session pour savoir ce qu'un compte
              valait. ⚠️ « effectif » et non « posé » : c'est ce que veveprice
              APPLIQUERA, donc la seule valeur qui répond à la question. */ ''}
        palier : <b>${echapper(c.palier_effectif)}</b>${
      c.palier_pose
        ? ` <span class="doux">(posé${c.palier_jusqu_a ? `, jusqu’au ${echapper(c.palier_jusqu_a.slice(0, 10))}` : ''})</span>`
        : ' <span class="doux">(de son abonnement)</span>'}<br>
      ...
      ${/* ═══════════════════════════════════════════════════════════════════
            🔴🔴 LOT 163-A — POSER UN PALIER, ET IL PORTE SA DATE DE FIN.
            ═══════════════════════════════════════════════════════════════
            ⛔ LA DURÉE N'EST PAS UN CHAMP OPTIONNEL QU'ON POURRAIT VIDER : une
            surcharge sans fin est exactement ce qu'`access.mjs` refuse par
            écrit (« rien ne me rappellera de l'éteindre »). Le champ est
            `required`, et la borne qui compte est dans `server.ts`.
            ⭐ LA LISTE N'OFFRE QUE LES PALIERS QUI OUVRENT QUELQUE CHOSE.
            `visitor`, `free` et `member` sont au rang de `member` ou en
            dessous : les poser ne changerait RIEN (paliDe prend le maximum),
            et un geste qui ne fait rien sans le dire coûte une heure de
            recherche. `accorderPalier` les refuse aussi — l'interface ne
            protège rien, elle se contente de ne pas tendre le piège.
            ⚠️ « Retirer » est un SECOND formulaire, pas une valeur du premier.
            Un « 0 » tapé dans le champ des jours retirerait la surcharge sans
            que rien ne l'annonce ; un bouton nommé « Retirer » dit ce qu'il
            fait avant qu'on clique. */ ''}
      <form method="post" action="/admin/palier" class="rang" style="margin-top:8px;gap:8px">
        <input type="hidden" name="ref" value="${echapper(c.ref)}">
        <select name="palier" aria-label="palier à poser">
          <option value="crevette">crevette</option>
          <option value="langouste">langouste</option>
          <option value="whale">whale</option>
        </select>
        <input name="jours" type="number" min="1" max="400" value="30" required
               style="width:90px" aria-label="jours de palier">
        <button class="principal">Poser</button>
      </form>
      ${c.palier_pose ? `<form method="post" action="/admin/palier" class="rang" style="margin-top:8px">
        <input type="hidden" name="ref" value="${echapper(c.ref)}">
        <input type="hidden" name="jours" value="0">
        <button>Retirer le palier posé</button>
      </form>` : ''}
    </div>`).join('');
  }
