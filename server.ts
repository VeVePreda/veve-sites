    ...
    if (p === '/admin' || p.startsWith('/admin/')) {
      ...
      if (attendu && !trop(REGLES.admin)) {
        ...
        if (ouvert) {
          ...
          /**
           * ═══════════════════════════════════════════════════════════════
           * 🔴🔴🔴 LOT 163-A — POSER UN PALIER. LE SECOND GESTE QUI ÉCRIT.
           * ═══════════════════════════════════════════════════════════════
           * CE QUE ÇA RÉPARE : `paliDe()` ne savait dire que `member` ou
           * `crevette`. `langouste` et `whale` étaient déclarés dans le
           * manifeste de veveprice, vendus sur `/offre/` (historique 30 j,
           * 90 j, Max, module `wallet_watch`) et **atteignables par personne
           * au monde**. C'est la panne du lot 122, un cran plus haut : elle a
           * été réparée pour `crevette` et laissée entière au-dessus.
           *
           * ⛔ MÊMES GARDE-FOUS QUE `/admin/abonner`, ET UN DE PLUS :
           *   1. le bloc `/admin` exige déjà le cookie d'exploitation et
           *      passe par `trop(REGLES.admin)` — rien n'est assoupli ;
           *   2. les jours sont BORNÉS. ⚠️ ICI LA BORNE BASSE EST **0**, pas
           *      1, parce que `0` est le geste de RETRAIT. C'est la seule
           *      différence avec la borne d'`/admin/abonner`, et elle est
           *      délibérée : sans elle, une surcharge posée ne pourrait plus
           *      être retirée avant sa date de fin — donc « me donner whale
           *      pour regarder » coûterait 400 jours de whale ;
           *   3. le palier est validé par `accorderPalier` contre `PALIERS`,
           *      et refusé s'il est au rang de `member` ou en dessous ;
           *   4. le compte est retrouvé par sa RÉFÉRENCE OPAQUE, jamais par
           *      un terme transporté dans la page.
           * ⭐ La trace dit la DURÉE et le PALIER, jamais la référence ni le
           *   terme — même règle que les deux gestes au-dessus.
           */
          if (m === 'POST' && p === '/admin/palier') {
            const b = await corpsDe(req);
            const ref = String(b.get('ref') ?? '');
            const palier = String(b.get('palier') ?? '');
            /**
             * 🔴🔴🔴 LE BANC NEUF A TROUVÉ CE TROU DANS MA PREMIÈRE ÉCRITURE,
             * et il vaut d'être raconté : j'avais écrit `Number(b.get('jours')
             * ?? -1)` avec, juste au-dessus, un commentaire disant « on exige
             * la forme, on ne la devine pas ». `Number('')` vaut **0**, et
             * dans CETTE route `0` n'est pas une valeur vide : c'est le geste
             * de RETRAIT. Un champ laissé vide retirait donc la surcharge, en
             * silence, en rendant `200` et un message de succès.
             * ⭐⭐⭐ *Le commentaire décrivait l'intention, pas le code.* La
             *   borne `Number.isInteger` ne pouvait rien voir : 0 est un
             *   entier parfaitement valide.
             * ⇒ ON VALIDE LA CHAÎNE AVANT DE LA CONVERTIR. `-1` est une
             *   sentinelle qui tombe hors bornes et se fait refuser avec son
             *   message, au lieu de se faire arrondir vers un geste.
             */
            const brutJours = String(b.get('jours') ?? '').trim();
            const jours = /^[0-9]{1,3}$/.test(brutJours) ? Number(brutJours) : -1;
            let message: string;
            if (!Number.isInteger(jours) || jours < 0 || jours > 400) {
              message = 'Durée refusée : un entier de 0 à 400 jours (0 retire le palier posé).';
            } else {
              message = accorderPalier(ref, palier, jours);
            }
            console.log(`[admin] palier (${jours === 0 ? 'retrait' : `${palier}, ${jours} j`}) : ${message}`);
            return html(res, pageAdmin(forme(), parSite(), activite(), undefined, message));
          }
